...
-- ============================================
-- This creates RLS policies for all 25 tables
-- Run this in Supabase SQL Editor after creating tables

-- ============================================
-- IMPORTANT NOTES:
-- ============================================
-- 1. Tables already have RLS ENABLED from schema.sql
-- 2. These policies assume Supabase Auth is being used
-- 3. auth.uid() returns the authenticated user's ID
-- 4. Service role bypasses RLS automatically
-- 5. For admin access, we check if user exists in users table (you can add role field later)
...
-- POLICY SUMMARY:
-- 
-- USER-OWNED DATA (users can CRUD their own):
-- ✅ users (select, update, insert)
-- ✅ kyc_documents (select, insert, update pending)
-- ✅ user_preferences (full CRUD)
-- ✅ deposits (select, insert, update pending)
-- ✅ withdrawals (select, insert, update pending)
-- ✅ positions (full CRUD)
-- ✅ trade_history (select, insert)
-- ✅ notifications (full CRUD)
-- ✅ auto_trader_configs (full CRUD)
-- ✅ price_alerts (full CRUD)
-- ✅ user_investments (select, insert)
-- ✅ sell_requests (select, insert)
-- ✅ support_tickets (select, insert, update)
-- ✅ ticket_messages (select own, insert to own)
-- ✅ crm_messages (select own)
-- ✅ transactions (select own)
-- 
-- PUBLIC DATA (everyone can read):
-- ✅ trading_signals (active only)
-- ✅ market_assets (enabled only)
-- ✅ investment_offers (enabled only)
-- ✅ payment_methods (enabled only)
-- ✅ admin_settings (all)
-- 
...
-- ✅ activity_logs
-- 
-- NEXT STEP: Test policies with actual queries!
-- ============================================
