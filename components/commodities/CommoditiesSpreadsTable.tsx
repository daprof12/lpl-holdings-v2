...
import { commoditySpreads, type CommoditySpread } from '../../data/commoditiesData';

type TabKey = 'all' | 'energy' | 'metals' | 'softs';
...
export default function CommoditiesSpreadsTable() {
  const [tab,   setTab]   = useState<TabKey>('all');
  const [query, setQuery] = useState('');

  const filtered = useMemo<CommoditySpread[]>(() => {
    const q = query.toLowerCase().trim();
    return commoditySpreads.filter(
      (s) =>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="bg-[#34e834] rounded-md px-5 py-2.5">
            <span className="font-medium text-gray-900 text-base">Commodities CFDs</span>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-400 mt-2">
          Showing {filtered.length} commodity instrument{filtered.length !== 1 ? 's' : ''}
        </p>
      </div>
    </section>
  );
