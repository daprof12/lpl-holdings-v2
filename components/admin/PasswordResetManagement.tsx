...
export default function PasswordResetManagement() {
  ...
  const loadRequests = async () => {
    setIsLoading(true);
    try {
      const data = await api.passwordResets.getAll();
  ...
  useEffect(() => {
    ...
    // Subscribe to password_resets changes
    const subscription = supabase
      .channel('password_resets_changes')
  ...
  const confirmSendRecoveryCode = async () => {
    ...
    try {
      await api.passwordResets.update(selectedRequest.id, {
  ...
  const handleRejectRequest = async (request: PasswordResetRequest) => {
    try {
      await api.passwordResets.update(request.id, { status: 'rejected' });
